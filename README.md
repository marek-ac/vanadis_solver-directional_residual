...
```math
Pe =
\frac{\left\|\mathbf{v}\right\|\,h_{\mathrm{stream}}}
     {2\lambda_s} .
```

The Directional Residual parameter is
...
The implementation includes numerically safe limiting forms for small and large Peclet numbers. In the pure-advection limit,
...
The diffusion part intentionally remains the standard Galerkin contribution

```math
\nabla N_i^{T}\Lambda\nabla N_j .
```

The Directional Residual perturbation enters the transient, advective, reaction/source, and applicable boundary weighting through `W_i`.
...
Vanadis v2026.3.1 uses an implicit time-integration scheme with
...
For atmospheric transport problems covered by the present model, a new application normally changes the **data and physical-field definitions**, not the FEM/DR core.

Typical case-specific inputs include:
