...
! See LICENSE.md for 
!
! VANADIS.FOR - VER. 1.08_1  14.08.2026
!
...
c------------------------------------------------------------
     ...
     &                               tnw2, mass)
         ...
         print *, 'Czas podzialu plikow = ', tsplit2 - tsplit1, ' sekund'
...
105      format('output\T_', i5.5, '.TXT')
         ...
         do i = 1, ilosc_wezlow
            ...
            write(bufor(ibuf), 122) xx(i), yy(i), zz(i),
     &                             tnw2(i) * 1d6
            ...
            if (ibuf .eq. nbuf) then
               ...
               do jbuf = 1, ibuf
                  ...
                  blok(ipos:ipos+lrec-1) = bufor(jbuf)
                  ipos = ipos + lrec

                  blok(ipos:ipos) = char(13)
                  ipos = ipos + 1

                  blok(ipos:ipos) = char(10)
                  ipos = ipos + 1

               enddo

               write(nout10, '(a)', advance = 'NO')
     &              blok(1:ipos-1)
...
! --- Dyfuzja efektywna w kierunku strugi
!     lambda_s = e^T * Lambda * e
         lambda_s = lambda(1)*ex*ex
     &            + lambda(2)*ey*ey
     &            + lambda(3)*ez*ez
...
!     Dla malego Pe uzywamy szeregu, dla duzego Pe granicy coth(Pe)=1
         tau_DR = 0.0d0
         if (Pe .gt. 1.0d-12 .and. vmag .gt. eps) then
            if (Pe .lt. 1.0d-3) then
               betaPe = Pe/3.0d0 - Pe**3/45.0d0
     &                + 2.0d0*Pe**5/945.0d0
            else if (Pe .gt. 50.0d0) then
               betaPe = 1.0d0 - 1.0d0/Pe
            else
