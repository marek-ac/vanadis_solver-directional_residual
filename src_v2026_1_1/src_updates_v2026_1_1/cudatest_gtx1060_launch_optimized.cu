...
extern "C" void kernel_dbcg_simple_(int *Np, double *B, double *X, double *TOLp,
  ...
  // CUDA part
  //****************************************************************************************
  //      const int threadsPerBlock = 256*2;
  //      const int blocksPerGrid   = imin( 32, (N+threadsPerBlock-1) /
  //      threadsPerBlock );

  // GTX 1060 / CC 6.1 launch geometry.  Detect the number of SMs at
  // runtime, so the same binary/source adapts to 6 GB and 3 GB cards.
  int cuda_device = 0;
  ...
  // Simple vector kernels are bandwidth-oriented.  256 threads/block and
  // up to eight blocks per SM expose enough independent warps on Pascal.
  ...
  // MATVEC/MTTVEC are much heavier per thread (64 FP64 contributions and
  // global FP64 atomics), so deliberately use fewer blocks than vector
  // kernels to avoid excessive atomic contention.
  int Mthreads = GTX1060_MATVEC_THREADS;
  int Mblocks = imin(sm_count * GTX1060_MATVEC_BLOCKS_PER_SM,
                     (ILE + Mthreads - 1) / Mthreads);
  if (Mblocks < 1) Mblocks = 1;

  // Dirichlet kernels are light, bandwidth-oriented vector kernels.
  ...
  // Reductions: use four 512-thread blocks per SM on GTX 1060.  This can
  // expose more warps for the FP64-heavy reduction while still reducing
  // final lock acquisitions versus the original fixed 64-block launch.
  ...
  */
  ...
  MATVEC_C_cu<<<Mblocks, Mthreads>>>(lock, X_cu, R_cu, ISS_cu, BB_cu, NAX, NAY, NAZ);
  ...
  for (K = 1; K <= ITMAX; K++) {
    ...
    *ITERp = ITER;
    ...
    MATVEC_C_cu<<<Mblocks, Mthreads>>>(lock, P_cu, Z_cu, ISS_cu, BB_cu, NAX, NAY,
                                 NAZ);
    ...
    MTTVEC_C_cu<<<Mblocks, Mthreads>>>(lock, PP_cu, ZZ_cu, ISS_cu, BB_cu, NAX, NAY,
                                 NAZ);
